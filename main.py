...
async def create_job(
    ...
    db_job = models.Job(title=title.strip(), description=final_desc, jd_filepath=file_path)
    db.add(db_job)
    db.commit()
    db.refresh(db_job)
    return db_job
...
async def upload_candidate(
    ...
    # Save with unique name — no overwrite
    ...
    # Recalculate everyone's rank in this job
    recalculate_ranks(job_id, db)
    db.refresh(candidate)
